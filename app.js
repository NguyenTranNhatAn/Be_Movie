  ...
  socket.on('select_seat', async ({ showtimeId, row, col }) => {
    ...
    if (roomShapeArray[row][col] === 'T' || roomShapeArray[row][col] === 'V' || roomShapeArray[row][col] === 'D') {
      ...
      if (roomShapeArray[row][col] === 'D') {
        ...
        roomShapeArray[row][col] = 'P'; // Đặt ghế thành Pending
        showtime.Room_Shape = roomShapeArray.map(row => row.join('')).join('/');
        await showtime.save();

        io.emit('seat_selected', { row, col }); // Thông báo cho tất cả người dùng khác
      }
    } else {
  ...
  socket.on('pay_for_seats', async ({ showtimeId, seats }) => {
    ...
    seats.forEach(({ row, col }) => {
      if (roomShapeArray[row][col] === 'P') {
        roomShapeArray[row][col] = 'U'; // Đặt ghế thành đã đặt
      }
    });
    ...
    // Gửi thông báo cập nhật cho tất cả người dùng
    seats.forEach(({ row, col }) => {
      ...
      if (roomShapeArray[row][col] === 'D') {
        ...
        seatTimers[seatId] = setTimeout(async () => {
          ...
          if (updatedShowtime) {
            ...
            // Revert seat back to original state
            updatedRoomShapeArray[row][col] = roomShapeArray[row][col] === 'T' ? 'T' : 'V'; // Revert to original type
